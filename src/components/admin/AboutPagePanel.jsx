...
export default function AboutPagePanel() {
  ...
  // Derived state slices (kept in sync with pageData)
  ...
  const saveStat = async (section) => {
    ...
    try {
      ...
      if (editingStat.item.id) {
        ...
        if (section === "who_we_are") setWhoStats((p) => [...p, r.data]);
        else setBannerStats((p) => [...p, r.data]);
      }
      setEditingStat(null);
    } catch (err) {
      setFormErr(err.message);
  ...
  const delStat = (id, section) =>
    askDel("Delete this stat?", async () => {
      ...
      else setBannerStats((p) => p.filter((x) => x.id !== id));
    });
      ...
      <div className="space-y-4">
          ...
          <div className="space-y-4">
            <div className="grid sm:grid-cols-2 gap-4">
              ...
              <ImgSlot
                ...
                onUpload={(m) =>
                  setMeta((p) => ({ ...p, why_choose_image: m }))
                }
                onClear={() =>
                  setMeta((p) => ({ ...p, why_choose_image: null }))
                }
              />
            </div>
          ...
          <div className="space-y-4">
            <div className="grid sm:grid-cols-2 gap-4">
              ...
              <ImgSlot
                ...
                onUpload={(m) =>
                  setMeta((p) => ({ ...p, who_we_are_image: m }))
                }
                onClear={() =>
                  setMeta((p) => ({ ...p, who_we_are_image: null }))
                }
              />
            </div>
        ...
        >
          <div className="space-y-4">
            ...
            <GoldBtn onClick={saveMeta} disabled={savingMeta}>
              {savingMeta ? (
                <>
                  <Loader2 size={14} className="animate-spin" /> Saving…
                </>
        ...
        <SCard
          title="CTA Section"
          subtitle="Bottom call-to-action section — background, heading and buttons"
          onSave={saveMeta}
          saving={savingMeta}
          status={statusMeta}
        >
